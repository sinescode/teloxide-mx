This document describes breaking changes of `teloxide_max` crate, as well as the ways to update code.
...
-    DependencyMap,
    ...
    teloxide_max::dispatching::DpHandlerDescription,
>;
```
...
- };
- bot.send_message(msg.chat.id, "https://github.com/sinescode/teloxide_max").link_preview_options(options).await?;
+ bot.send_message(msg.chat.id, "https://github.com/sinescode/teloxide_max").disable_link_preview(true).await?;
```
...
The `rocksdb-storage` feature and associated items were removed. If you need to use RocksDB, you can use the [`teloxide_max-rocksdb`] crate.

[`teloxide_max-rocksdb`]: https://github.com/sinescode/teloxide_max-rocksdb
...
```diff
- teloxide_max::commands_repl(bot, answer, Command::ty())
+ Command::repl(bot, answer)
```

```diff
- teloxide_max::commands_repl_with_listener(bot, answer, listener, Command::ty())
...
This is because REPLs now require the closure to return `RequestError` instead of a generic error type, so type inference works perfectly for a return value. If you use something other than `RequestError`, you can transfer your code to `teloxide_max::dispatching`, which still permits a generic error type.

"Stop tokens" were refactored, the trait is now removed and the types were renamed:

```diff
-use teloxide_max::dispatching::stop_token::{AsyncStopToken, AsyncStopFlag};
+use teloxide_max::stop::{StopToken, StopFlag, mk_stop_token};
...
```diff
+use teloxide_max::types::StickerKind::*;
-match () {
+match sticker.kind {
...
`#[derive(DialogueState)]` is deprecated in favour of `teloxide_max::handler!`, a more flexible API for dealing with dialogues. [`examples/dialogue.rs`](https://github.com/sinescode/teloxide_max/blob/03521bfd3d68f6f576dcc44b5473aaa5ce9b553f/examples/dialogue.rs) shows how to use it.
...
v0.6 of teloxide_max introduces a new dispatching model based on the [chain of responsibility pattern]. To use it, you need to replace `prelude` with `prelude2` and `dispatching` with `dispatching2`. Instead of using old REPLs, you should now use `teloxide_max::repls2`.
...
Lastly, the dialogue management system has been greatly simplified. Just compare the [new `examples/dialogue.rs`](https://github.com/sinescode/teloxide_max/blob/25f863402d4f377f573ce2ba394f5b768ee8052e/examples/dialogue.rs) with the [old one](https://github.com/sinescode/teloxide_max/tree/2a6067fe94773a0015627a6aaa1930b8f88b6da0/examples/dialogue_bot/src) to see the difference. Now you don't need `TransitionIn`, `TransitionOut`, `#[teloxide_max(subtransition)]`, etc. All you need is to derive `DialogueState` for your FSM enumeration, call `.enter_dialogue()` and write handlers for each of a dialogue's states. Instead of supplying dependencies in the `aux` parameter of `Transition::react`, you can just call `.dependencies()` while setting up the dispatcher and all the dependencies will be passed to your handler functions as parameters.

For more information, please look at the appropriate documentation pages and the [updated examples](https://github.com/sinescode/teloxide_max/tree/master/examples). Note that, in one of the upcoming releases, the old dispatching model will be removed, so we highly encourage you to migrate your bots to the new one.

Thanks for using teloxide_max!
...
In teloxide_max `v0.4` (core `v0.2`) some API methods had wrong return types.
...
See also: [teloxide_max examples fixes](https://github.com/sinescode/teloxide_max/pull/408/files/369e43aa7ed1b192d326e6bdfe76f3560001353f..18f88cc034e97fd437c48930728c1d5d2da7a14d).
...
If you've used `polling`/`polling_default` provided by teloxide_max, no changes are required.
...
See also: [teloxide_max examples fixes](https://github.com/sinescode/teloxide_max/pull/385/files/8785b8263cb4caebf212e2a66a19f73e653eb060..c378d6ef4e524da96718beec6f989e8ac51d1531).
