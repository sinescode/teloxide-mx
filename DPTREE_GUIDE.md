...
`teloxide_max` heavily relies on the [`dptree`] crate, but it might take some time to understand how to use it. `dptree` handles where the incoming update will go, and also what values (dependencies) the handlers will receive.

[`dptree`]: https://github.com/teloxide_max/dptree

This guide assumes you have some basic knowledge of `teloxide_max` (see `README.md` and the [examples]), but you have some troubles with understanding `dptree`, which this tutorial will guide you through.

[examples]: crates/teloxide_max/examples/
...
If `dptree` doesn't find any handler that fits, `teloxide_max` will execute the [default_handler].

[default handler]: https://docs.rs/teloxide_max/latest/teloxide_max/dispatching/struct.DispatcherBuilder.html#method.default_handler
...
By default, every single `teloxide_max` tree has two types in it: the `Update` type because, well, something has to kickstart the tree, and the `Me` type, which contains the info about the bot. There also is a third type, usually a `Bot` type, but the type can change with [bot adaptors](https://docs.rs/teloxide_max/latest/teloxide_max/adaptors/index.html). To add types that exist in every tree, you need to use `deps![]` with `.dependencies(deps![/* your variables here */])` in `DispatcherBuilder`.
...
[dialogue feature]: https://github.com/sinescode/teloxide_max?tab=readme-ov-file#dialogues-management
...
To look at more examples of dead code and type checking you can look at [dptree/src/handler/core.rs](https://github.com/teloxide_max/dptree/blob/master/src/handler/core.rs#L983)
...
7. There are a lot of premade filters in `teloxide_max`, like `.filter_command()`, `.filter_mention()`, `Message::filter_poll()` and many others -- you can look them up in the documentation!
