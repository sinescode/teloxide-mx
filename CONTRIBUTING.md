...
To report a bug or suggest new functionality, go to the [issues](https://github.com/sinescode/teloxide_max/issues). Try to make MRE (**M**inimal **R**eproducible **E**xample) and specify your `teloxide_max` version to let others help you.

If you want to ask a question, you can either
- open a new [GitHub discussion](https://github.com/sinescode/teloxide_max/discussions), or
- write to our Telegram group ([ENG](https://t.me/teloxide_max), [RU](https://t.me/teloxide_max_ru)).

If you want to contribute a new feature or a TBA update, you should contact other developers in the [teloxide_max_dev](https://t.me/teloxide_max_dev) chat, or in the github issues. What you want to do may already be in the works, or we may not want to support your feature in the future. 
...
[GitHub issue]: https://github.com/sinescode/teloxide_max/issues
...
If you want, you can develop `teloxide_max` inside a DevPod container. You can read more about DevPod [here](https://devpod.sh/docs/what-is-devpod).

Option 1:

```shell
git clone <your-fork> teloxide_max
cd teloxide_max
devpod up .
```
...
```shell
devpod up https://github.com/sinescode/teloxide_max
```
...
You start at the [official TBA release notes](https://core.telegram.org/bots/api-changelog), BUT you should use [web archive](https://web.archive.org/) if `teloxide_max` is lagging behind a few updates. If you use the latest docs you might add something that is not yet supported in the rest of the codebase!
...
And all new types should be added to `teloxide_max_core/src/types/` as a new file, if needed.
...
You need to update the check schema. It is located in `teloxide_max_core/custom_v2.json`. To obtain it, you need to clone the repository https://github.com/teloxide_max/tba-parser and generate a new schema. Follow the repository README to get the schema and copy the `schema/custom_v2.json` to `teloxide_max_core/custom_v2.json`, then run the tests!
...
If there was a release lately, you should probably change the `teloxide_max/Cargo.toml` to use the path for `teloxide_max_core` and `teloxide_max_macros`, so they reference the latest code
...
- Specify your changes in [crates/teloxide_max_core/CHANGELOG.md](crates/teloxide_max_core/CHANGELOG.md) file
...
- Change TBA version and it's announce date in `api_version: ApiVersion(ver: "7.0", date: "December 29, 2023"),` line in head of [crates/teloxide_max_core/schema.ron](crates/teloxide_max_core/schema.ron) file
- Change TBA version in `(Currently, version … is supported)` line in head of [crates/teloxide_max_core/src/lib.rs](crates/teloxide_max_core/src/lib.rs) file
- Change TBA version in `Currently, version … of` line in head of [crates/teloxide_max/src/lib.rs](crates/teloxide_max/src/lib.rs) file
- Change TBA version in `…https://img.shields.io/badge/API%20coverage…` line in [crates/teloxide_max_core/README.md](crates/teloxide_max_core/README.md) file
...
Add the method and its info to `crates/teloxide_max_core/src/schema.ron` file.

For example, lets add a `createChatInviteLink` from TBA 5.1. Look at the [TBA documentation](https://web.archive.org/web/20210331031440/https://core.telegram.org/bots/api#createchatinvitelink) in the web archive (`teloxide_max` can lag behind a few versions of TBA, you should look at the past versions to not implement something that isn't in the version you are doing).
...
Rerun the tests, and look at the compiler errors. Most likely there will be errors from `crates/teloxide_max_core/src/adaptors`, `crates/teloxide_max_core/src/requests/requester.rs` and `crates/teloxide_max_core/src/bot/api.rs`.
...
3. Sometimes you will encounter situation when it's impossible to write something in `schema.ron`. It may be some doc comment that renders incorrectly or some type (like `Option<DateTime<Utc>>`) impossible to write. In such cases you can workaround it with `crates/teloxide_max_core/src/codegen/patch.rs` file.

4. In very rare cases, autogenerated files in `crates/teloxide_max_core/src/payloads/*` may have errors related to deriving impls for the `PartialEq`, `Eq`, or `Hash` traits. In such cases, remove them manually and add the payload as an exception to `codegen_payloads()` in `crates/teloxide_max_core/src/payloads/codegen.rs`.

#### Other notes

1. If you mess up the .ron and run the codegen, it is better to reset the files, rather than to try and fix it all by hand:
`git restore crates/teloxide_max_core/src/local_macros.rs crates/teloxide_max_core/src/payloads.rs crates/teloxide_max_core/src/payloads/ crates/teloxide_max_core/src/requests/requester.rs && git clean -fd crates/teloxide_max_core/src/payloads/`
...
`teloxide_max` uses @teloxide_maxbot as a helper to manage PRs and issues. It is based on triagebot used by rustc developers, which docs can be found [here](https://forge.rust-lang.org/triagebot/index.html).

We will describe here a few most used @teloxide_maxbot's features, but we still recommend you to read the docs.

### PR status tracking

`teloxide_max` uses `S-*` labels (mainly https://github.com/sinescode/teloxide_max/labels/S-waiting-on-author and https://github.com/sinescode/teloxide_max/labels/S-waiting-on-review) to track the status of pull requests.

You can change the status with `@teloxide_maxbot review` and `@teloxide_maxbot ready` (sets the status to https://github.com/sinescode/teloxide_max/labels/S-waiting-on-review) or `@teloxide_maxbot author` (sets the status to https://github.com/sinescode/teloxide_max/labels/S-waiting-on-author).
...
There is also https://github.com/sinescode/teloxide_max/labels/S-blocked, which can be set with `@teloxide_maxbot blocked`.

Please note that your PR won't be reviewed unless it's waiting for review :)

### Labels

Normally, GitHub only allows privileged users to change labels. @teloxide_maxbot allows anyone to add or remove certain [labels](https://github.com/sinescode/teloxide_max/labels/) with `@teloxide_maxbot label +additional_label -removed_label`. See more in the [documentation](https://forge.rust-lang.org/triagebot/index.html).

### PR assignment

When a PR is created, @teloxide_maxbot will automatically assign one of the maintainers to it. If you want to override this assignment, use `r? @ReviewerUsername`.
