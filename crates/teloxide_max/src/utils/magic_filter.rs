...
impl F {
    ...
    /// Filter for CallbackQuery fields.
    pub const CALLBACK: CallbackQueryFilter = CallbackQueryFilter;

    /// Filter for InlineQuery fields.
    pub const INLINE: InlineQueryFilter = InlineQueryFilter;
}
...
impl ComposedFilter {
    ...
    pub fn into_dptree(self) -> Box<dyn Fn(&Message) -> bool + Send + Sync> {
        Box::new(move |msg| self.matches(msg))
    }
}
...
/// Extension to make ComposedFilter usable as a dptree filter.
pub trait FilterIntoDptree<T> {
    /// Convert into a dptree-compatible filter function.
    fn into_dptree_filter(self) -> Box<dyn Fn(&T) -> bool + Send + Sync>;
}

impl FilterIntoDptree<Message> for ComposedFilter {
    fn into_dptree_filter(self) -> Box<dyn Fn(&Message) -> bool + Send + Sync> {
        Box::new(move |msg| self.matches(msg))
    }
}
