...
use teloxide_max_core::errors::AsResponseParameters;
        ...
        // Poll in-flight future until completion
            ...
            match res {
                ...
                Err(err) if *this.stopping => {
                    // Prevents infinite retries, see https://github.com/sinescode/teloxide_max/issues/496
                ...
                Err(err) => {
                    /*
                       ...
                       Otherwise, in case of network connection lose (<https://github.com/sinescode/teloxide_max/issues/780>) we
