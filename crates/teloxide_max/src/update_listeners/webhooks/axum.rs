...
pub fn axum_no_setup(
    ...
    async fn telegram_request(
        ...
        // Validate secret token with constant-time comparison (prevents timing attacks)
        let token_valid = match (&secret_header.0, &secret) {
            (Some(received), Some(expected)) => constant_time_eq(received, expected.as_bytes()),
            (None, None) => true,
            _ => false,
        };
        if !token_valid {
        ...
        match serde_json::from_str::<Update>(&input) {
            Ok(mut update) => {
                // See HACK comment in
                // `teloxide_max_core::net::request::process_response::{closure#0}`
            ...
            Err(error) => {
                log::error!(
                    "Cannot parse an update.\nError: {error:?}\nValue: {input}\n\
                     This is a bug in teloxide_max_core, please open an issue here: \
                     https://github.com/sinescode/teloxide_max/issues."
                );
            }
        };
