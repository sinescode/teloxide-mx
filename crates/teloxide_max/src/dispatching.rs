...
//!
//! In `teloxide_max`, update dispatching is declarative: it takes the form of a
...
//! ```no_run
//! # #[cfg(feature = "macros")] {
//! # use teloxide_max::utils::command::BotCommands;
...
//! # use teloxide_max::dispatching::{UpdateHandler, UpdateFilterExt, dialogue, dialogue::InMemStorage};
//! # use teloxide_max::utils::command::BotCommands;
...
//!     use dptree::case;
//!
//!     let command_handler = teloxide_max::filter_command::<Command, _>()
//!         .branch(
//!             case![State::Start]
...
//! # use teloxide_max::types::{Message, CallbackQuery};
...
//! ```
//!
//! Each parameter is supplied as a dependency by `teloxide_max`. In particular:
...
//! # use teloxide_max::Bot;
//! # use teloxide_max::requests::RequesterExt;
//! # use teloxide_max::dispatching::{Dispatcher, dialogue::InMemStorage};
//! # enum State {}
//! # fn schema() -> teloxide_max::dispatching::UpdateHandler<Box<dyn std::error::Error + Send + Sync + 'static>> { teloxide_max::dptree::entry() }
//! #[tokio::main]
//! async fn main() {
...
//! [dialogues]: dialogue
//! [`examples/purchase.rs`]: https://github.com/sinescode/teloxide_max/blob/master/crates/teloxide_max/examples/purchase.rs
...
//! [`examples/dispatching_features.rs`]: https://github.com/sinescode/teloxide_max/blob/master/crates/teloxide_max/examples/dispatching_features.rs
