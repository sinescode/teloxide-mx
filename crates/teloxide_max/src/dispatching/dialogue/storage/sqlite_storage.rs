    ...
    fn remove_dialogue(
        ...
        Box::pin(async move {
            let deleted_rows_count =
                sqlx::query("DELETE FROM teloxide_max_dialogues WHERE chat_id = ?")
                    .bind(chat_id)
                    .execute(&self.pool)
                    .await?
                    .rows_affected();
        ...
        Box::pin(async move {
            ...
            self.pool
                ...
                .execute(
                    sqlx::query(
                        "
            INSERT INTO teloxide_max_dialogues VALUES (?, ?)
...
async fn get_dialogue(
    ...
    let bytes = sqlx::query_as::<_, DialogueDbRow>(
        "SELECT dialogue FROM teloxide_max_dialogues WHERE chat_id = ?",
    )
    .bind(chat_id)
    .fetch_optional(pool)
    .await?
