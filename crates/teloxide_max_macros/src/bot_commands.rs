...
pub(crate) fn bot_commands_impl(input: DeriveInput) -> Result<TokenStream> {
    ...
    let trait_impl = quote! {
        impl teloxide_max::utils::command::BotCommands for #type_name {
            #fn_descriptions
            #fn_parse
            #fn_commands
        }
...
fn impl_commands(infos: &[Command]) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn bot_commands() -> ::std::vec::Vec<teloxide_max::types::BotCommand> {
            use teloxide_max::types::BotCommand;
            ::std::vec![#(#commands),*]
        }
    }
}
...
fn impl_descriptions(infos: &[Command], global: &CommandEnum) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn descriptions() -> teloxide_max::utils::command::CommandDescriptions<'static> {
            use teloxide_max::utils::command::{CommandDescriptions, CommandDescription};
            use std::borrow::Cow;
...
fn impl_parse(
    ...
    quote! {
         fn parse(s: &str, bot_name: &str) -> ::std::result::Result<Self, teloxide_max::utils::command::ParseError> {
              // FIXME: we should probably just call a helper function from `teloxide_max`, instead of parsing command syntax ourselves
              use std::str::FromStr;
              use teloxide_max::utils::command::ParseError;
