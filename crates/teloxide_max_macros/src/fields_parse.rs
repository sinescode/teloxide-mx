                    ...
                    (
                        |s: ::std::string::String| {
                            let res = <#ty>::from_str(&s)
                                .map_err(|e| teloxide_max::utils::command::ParseError::IncorrectFormat(e.into()))?;
    ...
    let res = {
        ...
        quote! {
            (
                #(
                    {
                        let s = splitted.next().ok_or(teloxide_max::utils::command::ParseError::TooFewArguments {
                            expected: #expected,
                            found: #found,
                            message: format!("Expected but not found arg number {}", #found + 1),
                        })?;

                        <#types>::from_str(s).map_err(|e| teloxide_max::utils::command::ParseError::IncorrectFormat(e.into()))?
                    },
                )*
            )
        }
    ...
    let res = quote! {
        (
            |s: ::std::string::String| {
                ...
                match splitted.next() {
                    Some(d) if !s.is_empty() => ::std::result::Result::Err(teloxide_max::utils::command::ParseError::TooManyArguments {
                        expected: #expected,
