//! Core part of the [`teloxide_max`] library.
//!
...
//!
//!```toml
//! teloxide_max_core = "0.13.0"
//! ```
//! _Compiler support: requires rustc 1.85+_.
//!
//! ```
//! # async {
//! # let chat_id = teloxide_max_core::types::ChatId(-1);
...
//
// `dep_docsrs` is used for the same purpose, but when `teloxide_max_core` is built as a dependency
// (see: `teloxide_max`). We can't use `docsrs` as it breaks tokio compilation in this case.
