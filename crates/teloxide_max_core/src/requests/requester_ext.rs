...
use crate::adaptors::request_hooks::{RequestHooks, RequestHooksAdaptor};
...
pub trait RequesterExt: Requester {
    ...
    /// Add request hooks, see [`RequestHooksAdaptor`] for more.
    ///
    /// # Example
    ///
    /// ```rust,no_run
    ...
    #[cfg(feature = "request_hooks")]
    #[must_use]
    fn with_hooks(self, hooks: RequestHooks) -> RequestHooksAdaptor<Self>
    where
        Self: Sized,
    {
        RequestHooksAdaptor::new(self, hooks)
    }
