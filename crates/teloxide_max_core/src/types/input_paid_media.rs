...
pub enum InputPaidMedia {
    ...
    /// The paid media to send is a live photo. TBA 10.0+
    LivePhoto {
        /// Video component of the live photo
        media: InputFile,
...
impl From<InputPaidMedia> for InputFile {
    fn from(media: InputPaidMedia) -> InputFile {
        match media {
            ...
            InputPaidMedia::LivePhoto { media, .. } => media,
        }
    }
}
...
impl InputPaidMedia {
    ...
    pub(crate) fn files(&self) -> impl Iterator<Item = &InputFile> {
        ...
        let (media, thumbnail, photo) = match self {
            ...
            Video(input_paid_media_video) => {
                (&input_paid_media_video.media, input_paid_media_video.thumbnail.as_ref(), None)
            }
            LivePhoto { media, photo, .. } => (media, None, photo.as_ref()),
        };
    ...
    pub(crate) fn files_mut(&mut self) -> impl Iterator<Item = &mut InputFile> {
        ...
        let (media, thumbnail, photo) = match self {
            Photo(InputPaidMediaPhoto { media, .. }) => (media, None, None),
            Video(input_paid_media_video) => {
                (&mut input_paid_media_video.media, input_paid_media_video.thumbnail.as_mut(), None)
            }
            LivePhoto { media, photo, .. } => (media, None, photo.as_mut()),
        };
