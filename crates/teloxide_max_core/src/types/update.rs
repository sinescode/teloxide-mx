...
use crate::types::{
    ...
    InlineQuery, ManagedBotCreated, ManagedBotUpdated, Message, MessageReactionCountUpdated,
...
pub enum UpdateKind {
    /// User payment subscription has changed. TBA 10.2+
    Subscription(BotSubscriptionUpdated),
    /// A guest message was received. TBA 10.0+
    GuestMessage(Message),
    /// A managed bot was created. TBA 9.6+
    ManagedBotCreated(ManagedBotCreated),
    /// A managed bot was updated. TBA 9.6+
...
impl Update {
    ...
    pub fn mentioned_users(&self) -> impl Iterator<Item = &User> {
        ...
        match &self.kind {
            ...
            | UpdateKind::ManagedBotCreated(_)
            | UpdateKind::ManagedBotUpdated(_)
                ...
                A: MapAccess<'de>,
            {
                ...
                let this = key
                    ...
                    .and_then(|key| match key {
                        ...
                        "managed_bot_created" => map
                            .next_value::<ManagedBotCreated>()
                            .ok()
                            .map(UpdateKind::ManagedBotCreated),
                        "managed_bot_updated" => map
                            .next_value::<ManagedBotUpdated>()
                            .ok()
                            .map(UpdateKind::ManagedBotUpdated),
                        _ => Some(empty_error()),
                    })
...
impl Serialize for UpdateKind {
        ...
        S: Serializer,
    {
        ...
        match self {
            ...
            UpdateKind::Subscription(v) => s.serialize_newtype_variant(name, 23, "subscription", v),
            UpdateKind::GuestMessage(v) => {
                s.serialize_newtype_variant(name, 24, "guest_message", v)
            }
            UpdateKind::ManagedBotCreated(v) => {
                s.serialize_newtype_variant(name, 25, "managed_bot_created", v)
            }
            UpdateKind::ManagedBotUpdated(v) => {
                s.serialize_newtype_variant(name, 26, "managed_bot_updated", v)
            }
...
"#;
    ...
    "message": {
        "chat": {
            "id": -1001276785818,
            "title": "teloxide_max dev",
            "type": "supergroup",
            "username": "teloxide_max_dev"
        },
        "date": 1582134655,
        "from": {
            "first_name": "Hirrolot",
