...
pub(crate) fn callback_data_impl(input: DeriveInput) -> Result<TokenStream> {
            ...
            if v.fields.is_empty() {
                ...
                quote! {
                    Self::#ident { #(#field_names),* } => {
                        ...
                        if result.len() > 64 {
                            return Err(teloxide::utils::callback_data::CallbackDataError::TooLong {
                                len: result.len(),
                                max: 64,
                            });
                        }
    ...
    let serialize_fn = quote! {
        fn serialize(&self) -> ::std::result::Result<String, teloxide::utils::callback_data::CallbackDataError> {
            match self {
                #(#serialize_arms)*
            }
        }
                    ...
                    .map(|(name, ty)| {
                        ...
                        quote! {
                            let #name: #ty = parts.next()
                                .ok_or_else(|| teloxide::utils::callback_data::CallbackDataError::DeserializationError(
                                    format!("missing field '{}'", #name_str)
                                ))?
                                .parse()
                                .map_err(|e: ::std::num::ParseIntError| teloxide::utils::callback_data::CallbackDataError::DeserializationError(
                ...
                quote! {
                    #full => {
                        ...
                        if !rest.is_empty() && !rest.starts_with(#separator) {
                            return Err(teloxide::utils::callback_data::CallbackDataError::DeserializationError(
    ...
    let deserialize_fn = quote! {
        fn deserialize(data: &str) -> ::std::result::Result<Self, teloxide::utils::callback_data::CallbackDataError> {
            // Try each variant's full prefix (prefix + separator + suffix)
            match data {
                #(#deserialize_arms)*
                _ => Err(teloxide::utils::callback_data::CallbackDataError::DeserializationError(
    ...
    let impl_block = quote! {
        impl #lifetime teloxide::utils::callback_data::CallbackData for #type_name #lifetime #where_clause {
            #prefix_fn
            #separator_fn
            #serialize_fn
            #deserialize_fn
        ...
        impl #lifetime ::std::str::FromStr for #type_name #lifetime #where_clause {
            type Err = teloxide::utils::callback_data::CallbackDataError;
